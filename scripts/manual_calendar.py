#!/usr/bin/env python3
"""
经济日历手动输入脚本（临时方案）
在 FMP API 升级前，可以手动输入今日重点经济事件
"""

import json
import os
from datetime import datetime, timedelta

# 输出目录
OUTPUT_DIR = "/home/ubuntu/.openclaw/workspace/shared/02_outbox"

# 手动输入的经济日历数据（示例）
MANUAL_CALENDAR = [
    {
        "name": "FOMC 议息会议",
        "date": "2026-03-19",
        "time": "14:00",  # UTC 时间
        "country": "US",
        "importance": "High",
        "consensus": None,
        "actual": None,
        "previous": None,
        "note": "美联储 3 月利率决议，市场预计维持利率不变"
    },
    {
        "name": "美国 CPI (YoY)",
        "date": "2026-03-18",
        "time": "12:30",  # UTC 时间
        "country": "US",
        "importance": "High",
        "consensus": 3.0,
        "actual": None,
        "previous": 3.1,
        "note": "2 月消费者物价指数年率"
    },
    {
        "name": "美国非农就业人口",
        "date": "2026-03-21",
        "time": "12:30",  # UTC 时间
        "country": "US",
        "importance": "High",
        "consensus": 200,
        "actual": None,
        "previous": 275,
        "note": "2 月非农就业人口变动（千人）"
    },
    {
        "name": "中国 LPR 报价",
        "date": "2026-03-20",
        "time": "01:15",  # UTC 时间
        "country": "CN",
        "importance": "High",
        "consensus": 3.45,
        "actual": None,
        "previous": 3.45,
        "note": "中国 1 年期贷款市场报价利率"
    },
    {
        "name": "欧央行利率决议",
        "date": "2026-03-21",
        "time": "12:15",  # UTC 时间
        "country": "EU",
        "importance": "High",
        "consensus": 4.00,
        "actual": None,
        "previous": 4.00,
        "note": "欧洲央行主要再融资利率"
    },
]


def utc_to_beijing(utc_date: str, utc_time: str) -> str:
    """UTC 时间转北京时间"""
    from datetime import timedelta
    try:
        utc_dt = datetime.strptime(f"{utc_date} {utc_time}", "%Y-%m-%d %H:%M")
        beijing_dt = utc_dt + timedelta(hours=8)
        return beijing_dt.strftime("%Y-%m-%d %H:%M")
    except ValueError:
        return f"{utc_date} {utc_time}"


def format_event(event: dict) -> str:
    """格式化单个事件"""
    lines = []
    importance_emoji = {"High": "🔴", "Medium": "🟡", "Low": "🟢"}.get(event.get("importance", ""), "⚪")
    country_emoji = {"US": "🇺🇸", "CN": "🇨🇳", "EU": "🇪🇺", "UK": "🇬🇧", "JP": "🇯🇵"}.get(event.get("country", ""), event.get("country", ""))
    
    beijing_time = utc_to_beijing(event["date"], event["time"])
    
    lines.append(f"{importance_emoji} {event['name']}")
    lines.append(f"⏰ {beijing_time} (北京时间)")
    lines.append(f"🌍 {country_emoji}")
    lines.append(f"📊 重要性：{importance_emoji} {event.get('importance', 'Unknown')}")
    
    values = []
    if event.get("actual"):
        values.append(f"实际：{event['actual']}")
    if event.get("consensus"):
        values.append(f"预期：{event['consensus']}")
    if event.get("previous"):
        values.append(f"前值：{event['previous']}")
    
    if values:
        lines.append(" | ".join(values))
    
    if event.get("note"):
        lines.append(f"📝 {event['note']}")
    
    return "\n".join(lines)


def generate_briefing(events: list, target_date: str = None) -> str:
    """生成简报"""
    if target_date is None:
        target_date = datetime.now().strftime("%Y-%m-%d")
    
    lines = [
        f"📊 **经济日历简报** | {target_date}",
        "=" * 40,
        ""
    ]
    
    today = datetime.now().strftime("%Y-%m-%d")
    tomorrow = (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")
    
    # 按日期分组
    events_by_date = {}
    for event in events:
        event_date = event.get("date", today)
        if event_date not in events_by_date:
            events_by_date[event_date] = []
        events_by_date[event_date].append(event)
    
    for event_date in sorted(events_by_date.keys()):
        date_events = events_by_date[event_date]
        beijing_date = utc_to_beijing(event_date, "00:00").split()[0]
        
        # 判断日期标签
        if beijing_date == today:
            date_label = "📌 **今天**"
        elif beijing_date == tomorrow:
            date_label = "📌 **明天**"
        else:
            date_label = f"📌 **{beijing_date}**"
        
        lines.append(date_label)
        lines.append("-" * 30)
        
        # 按重要性排序
        importance_order = {"High": 0, "Medium": 1, "Low": 2}
        sorted_events = sorted(date_events, key=lambda x: importance_order.get(x.get("importance", "Low"), 2))
        
        for event in sorted_events:
            lines.append("")
            lines.append(format_event(event))
        
        lines.append("")
    
    lines.append("=" * 40)
    lines.append("⚠️ 数据来源：手动输入（FMP API 升级中）")
    
    return "\n".join(lines)


def main():
    """主函数"""
    import argparse
    
    parser = argparse.ArgumentParser(description="经济日历手动输入脚本")
    parser.add_argument("--output", type=str, help="输出文件路径")
    parser.add_argument("--date", type=str, help="目标日期 YYYY-MM-DD")
    parser.add_argument("--format", choices=["json", "text"], default="text", help="输出格式")
    
    args = parser.parse_args()
    
    target_date = args.date or datetime.now().strftime("%Y-%m-%d")
    
    # 生成简报
    briefing = generate_briefing(MANUAL_CALENDAR, target_date)
    
    if args.format == "json":
        output = json.dumps(MANUAL_CALENDAR, ensure_ascii=False, indent=2)
    else:
        output = briefing
    
    if args.output:
        os.makedirs(os.path.dirname(args.output), exist_ok=True)
        with open(args.output, 'w', encoding='utf-8') as f:
            f.write(output)
        print(f"✅ 已保存到：{args.output}")
    else:
        print(output)


if __name__ == "__main__":
    main()
